...
export class AgegroupTesterComponent implements OnInit {
  ...
  loadAgeGroups() {
    for (const age of this.ages) {
      const ageGroupStartDate = this.currentYear - age + "-07-01";
      const group = age + "U";
  ...
  calculateAgeGroup() {
    const birthdate: string =
      this.playerMonth + "/" + this.playerDay + "/" + this.playerYear;
    let ageGroup = "UNK";
    let ageGroupDisplay =
      birthdate +
      " would play in unavailable age group. Contact Club Director.";
