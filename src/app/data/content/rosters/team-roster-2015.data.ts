import { TeamRoster } from '../../../team-roster/TeamRoster';

export const TEAM_ROSTER_2015: TeamRoster[] = [
    {
        year: 2015,
        name: 'Carnelian',
        division: 'Club',
        ageGroup: '16U',
        headCoach: 'Todd Taylor',
        assistantCoaches: [],
        players: ['Badger, Jessica', 'Barnes, Virginia', 'Boyd, Lila', 'Caruso, Victoria', 'Desrochers, Gabrielle', 'Federico, Isabella', 'LaLumiere, McKenzie', 'MacInnes, Kaitlyn', 'Mucci, Laura', 'Sammui, Emma', 'Van Stry, Erica'],
        teamPhoto: '',
        teamPhotoThumbNail: ''
    },
    {
        year: 2015,
        name: 'Cerulean',
        division: 'Club',
        ageGroup: '14U',
        headCoach: 'Mona Goldthwaite',
        assistantCoaches: [],
        players: ['Bata, Taylor', 'Boese, Sara Beth', 'Coutu, Stella', 'Desai, Diya', 'Determan, Allison', 'Geary, Brynn', 'Goldthwaite, Neena', 'Hitchcock, Nicole', 'McNaughton, Jennifer', 'Murphy, Lauren'],
        teamPhoto: 'assets/img/teams/2015/cerulean.jpg',
        teamPhotoThumbNail: ''
    },
    {
        year: 2015,
        name: 'Crimson',
        division: 'Open',
        ageGroup: '18U',
        headCoach: 'Stephen Boyle',
        assistantCoaches: [],
        players: ['Blue, Ellie', 'Boyle, Jillian', 'Cataldo, Marissa', 'Farley, Hannah', 'Higgins, Karolyn', 'King, Mairin', 'McGourty, Marie', 'Rega, Allison', 'Smerczynski, Casey', 'Tang, Aubrey'],
        teamPhoto: 'assets/img/teams/2015/crimson.jpg',
        teamPhotoThumbNail: ''
    },
    {
        year: 2015,
        name: 'Ebony',
        division: 'Club',
        ageGroup: '15U',
        headCoach: 'Steve McRoy',
        assistantCoaches: ['Bonnie Zeigler'],
        players: ['Calnan, Sarah', 'Connolly, Saoirse', 'Fleming, Brooke', 'Fox, Morgan', 'Gaspar, Olivia', 'Karim, Julia', 'Kramer, Lauren', 'Meyers, Sophie', "O'Connor, Meghan", 'Steed, MaryRose', 'Wang, Sophia'],
        teamPhoto: '',
        teamPhotoThumbNail: ''
    },
    {
        year: 2015,
        name: 'Emerald',
        division: 'Club',
        ageGroup: '18U',
        headCoach: 'Bruce Shainwald',
        assistantCoaches: ['Bridget Shainwald'],
        players: ['Cooper, Katalinn','Cooper, Kelliann','Ezekiel, Jordan','Fortier, Danielle','Laverdiere, Julianna','Murray, Michelle','Nelson, Samantha','Nyarko-Dufie, Emily','Shainwald, Shannon','Thompson, Caroline'],
        teamPhoto: 'assets/img/teams/2015/Emerald.jpg',
        teamPhotoThumbNail: ''
    },
    {
        year: 2015,
        name: 'Heliotrope',
        division: 'Club',
        ageGroup: '15U',
        headCoach: 'Mikaela Mackinnon',
        assistantCoaches: [],
        players: ['Bessette, Hannah', 'Doherty, Hannah', 'Farris, Isabella', 'Gardner, Abaigeal', 'Gonzalez, Emely', 'Hamori, Phoebe', 'La Grassa, Catriona', 'MacEachern, Devyn', 'Maher, Allison', 'McPherson, Ali', 'Shahtanian, Anna'],
        teamPhoto: 'assets/img/teams/2015/heliotrope.jpg',
        teamPhotoThumbNail: ''
    },
    {
        year: 2015,
        name: 'Jade',
        division: 'Club',
        ageGroup: '18U',
        headCoach: 'Brian Healey',
        assistantCoaches: ['Nancy Hartmann'],
        players: ['Barbehenn, Sinclair', 'Camelo, Madison', 'Donahue, Ann', 'Donahue, Paige', 'Groele, Jamie', 'Mancini, Emma', 'Mullins, Colleen', 'Nason, Jessie', 'Nolan, Allie', 'Petherbridge, Nicole', 'Stankiewicz, Amy'],
        teamPhoto: 'assets/img/teams/2015/Jade.jpg',
        teamPhotoThumbNail: ''
    },
    {
        year: 2015,
        name: 'Jasper',
        division: 'Open',
        ageGroup: '18U',
        headCoach: 'Michele Naughton',
        assistantCoaches: ['Marla Marcum'],
        players: ['Boucher, Valerie', 'Butt, Lauren', 'Cullen, Delanie', 'Geary, Lauren', 'Griffin, Cailee', 'Kaplan, Sophia', 'Manuel, Kerry', "O'Connor, Maddi", 'Sandvos, Becki', 'Santhanakrishnan, Natasha', 'Slater, Abby'],
        teamPhoto: 'assets/img/teams/2015/jasper.jpg',
        teamPhotoThumbNail: ''
    },
    {
        year: 2015,
        name: 'Kryptonite',
        division: 'Club',
        ageGroup: '15U',
        headCoach: 'Dana Judge',
        assistantCoaches: [],
        players: ['Boyle, Maxine', 'Callahan, Shayla', 'Caputo, Grace', 'Fahey, Christa', 'Field, Lauren', 'Hamilton, Grace', 'Kukkal, Rhea', 'Kurr, Leah', 'Reitano, Morgan', 'Souza, Kayla', 'Tamura, Anna'],
        teamPhoto: '',
        teamPhotoThumbNail: ''
    },
    {
        year: 2015,
        name: 'Lazulite',
        division: 'Club',
        ageGroup: '18U',
        headCoach: 'Chanel Batiste',
        assistantCoaches: ['Danielle McDonald'],
        players: ['Barrett, Nicole', 'Brajak, Heidi', 'Cameron, Maddie', 'Coste, Carolyn', 'Cronin, Emily', "D'Addario, Lindsey", 'Goulet, Alyssa', "O'Neill, Emily", 'Pangione, Sophie', 'Sweeney, Mackenzie', 'Tomei, Lauren'],
        teamPhoto: 'assets/img/teams/2015/Lazulite.jpg',
        teamPhotoThumbNail: ''
    },
    {
        year: 2015,
        name: 'Maize',
        division: 'Club',
        ageGroup: '18U',
        headCoach: 'Joe Seid',
        assistantCoaches: [],
        players: ['Burliss, Madison', 'Butler, Nora', 'Cloutier, Theresa', 'Debreceni, Jamie', 'Fraser, Julia', 'Lessing, Merideth', 'Morelli, Jessica', 'Morgenstern, Allie', 'Paolella, Kendall', 'Ryan, Shannon'],
        teamPhoto: '',
        teamPhotoThumbNail: ''
    },
    {
        year: 2015,
        name: 'Mica',
        division: 'Club',
        ageGroup: '16U',
        headCoach: 'Miriam Moodley',
        assistantCoaches: [],
        players: ['Ada, Ruth', 'Bowler, Catherine', 'Casavant, Aimee', 'Cebi, Olivia', 'Connor, Colleen', 'Doyle, Sabrina', 'Hoehn, Greta', 'Malone, Emma', 'Perryman, Samantha', 'Reich, Clara'],
        teamPhoto: 'assets/img/teams/2015/mica.jpg',
        teamPhotoThumbNail: ''
    },
    {
        year: 2015,
        name: 'Midnight',
        division: 'Club',
        ageGroup: '16U',
        headCoach: 'Brianna Karpinski',
        assistantCoaches: [],
        players: ['Albanese, Rebecca', 'Amodei, Amelia', 'Chesmore, Keri', 'Kelly, Shaylyn', 'LaSpisa, Madison', 'Morin, Nicole', 'Ortega, Christine', 'Pinto, Paula', 'Snelson, Liz', 'Story, Mackenzie', 'Zhang, Vanessa'],
        teamPhoto: '',
        teamPhotoThumbNail: ''
    },
    {
        year: 2015,
        name: 'Obsidian',
        division: 'Club',
        ageGroup: '14U',
        headCoach: 'Melissa Wright',
        assistantCoaches: ['Fred Ahlholm'],
        players: ['Ahlholm, Lauren', 'Ahlholm, Samantha', 'Breton, Meaghan', 'Butler, Emily', 'Cheng, Cathy', 'Cunha, Arianna', 'Kirsch, Carly', 'Mitchell, Sophie', 'Morelli, Melissa', 'Morelli, Victoria'],
        teamPhoto: 'assets/img/teams/2015/obsidian.jpg',
        teamPhotoThumbNail: ''
    },
    {
        year: 2015,
        name: 'Onyx',
        division: 'Club',
        ageGroup: '18U',
        headCoach: 'Anny Din',
        assistantCoaches: ['Pedro Figueroa'],
        players: ['Chisholm, Delaney', 'Olan, Samantha', 'Donahue, Lindsay', 'Favreau, Adrianna', 'Godfroy, Victoria', 'Junek, Olivia', 'Murray, Theresa', 'Noyes, Christina', 'Robertson, Molly', 'Severino, Jasmine'],
        teamPhoto: 'assets/img/teams/2015/onyx.jpg',
        teamPhotoThumbNail: ''
    },
    {
        year: 2015,
        name: 'Pearl',
        division: 'Open',
        ageGroup: '16U',
        headCoach: 'Nancy Dumond',
        assistantCoaches: ['Rick Dumond', 'Rosella Latam'],
        players: ['Antczak, Katherine', 'Dykeman, Ashlie', 'Geary, Emma', 'Gotts, Brittany', 'Huang, Olivia', 'Latam, Olivia', 'McBride, Julia', 'McIntosh, Rachel', 'McNally, Megan', 'Zargaj, Julia', 'Zhang, Qiqi'],
        teamPhoto: '',
        teamPhotoThumbNail: ''
    },
    {
        year: 2015,
        name: 'Ruby',
        division: 'Open',
        ageGroup: '16U',
        headCoach: 'Jim Ray',
        assistantCoaches: ['Adina Ion'],
        players: ['Coen, Jen', 'Crowley, Ashley', 'Crowley, Shawna', 'Farnum, Kathryn', 'Feenstra, Rebecca', 'Kimball, Alessandra (Ali)', 'Moore, Kendra', 'Nassar, Natalie', 'Nugent, Kathryn', 'Stewart, Natasha'],
        teamPhoto: '',
        teamPhotoThumbNail: ''
    },
    {
        year: 2015,
        name: 'Saffron',
        division: 'Club',
        ageGroup: '14U',
        headCoach: 'Alicia Bursey',
        assistantCoaches: [],
        players: ['Danahy, Claire', 'Farnum, Laura', 'Farnum, Sarah', 'Gillman, Olivia', 'Harrison, Nicole', 'Keener, Jacqueline', 'King, Autumn', 'Lawton, Ashley', 'Parenti, Kristina', 'Sawka, Renee'],
        teamPhoto: 'assets/img/teams/2015/saffron.jpg',
        teamPhotoThumbNail: ''
    },
    {
        year: 2015,
        name: 'Sapphire',
        division: 'Club',
        ageGroup: '14U',
        headCoach: 'Mike Ezekiel',
        assistantCoaches: ['Jessica Torres'],
        players: ['Abouhamad, Brooke','Atwood, Emma','Cioto, Gianna','Cloutier, Marissa','Dand, Diya','Ezekiel, Elizabeth','Fraser, Caroline','Hu, Grace','Mitchell, Samantha','Pray, Lydia','Preller, Chris','Souza, Jenna'],
        teamPhoto: 'assets/img/teams/2015/sapphire_team.jpg',
        teamPhotoThumbNail: ''
    },
    {
        year: 2015,
        name: 'Steel Blue',
        division: 'Club',
        ageGroup: '16U',
        headCoach: 'Ali Martin',
        assistantCoaches: ['Annie Blanchette'],
        players: ['Bueno, Kiana', 'Cassidy, Jacqueline', 'Foley, Cameron', 'Huynh, Dalena', 'Maniscalco, Graciela', 'Martin, Fiona', 'McCloskey, Nicole', 'McGrath, Jennifer', 'Putsch, Daria', 'Shattuck, Lynn', 'Keiver, Abigail'],
        teamPhoto: '',
        teamPhotoThumbNail: ''
    },
    {
        year: 2015,
        name: 'Tanzanite',
        division: 'Club',
        ageGroup: '15U',
        headCoach: 'Kayla Greeley',
        assistantCoaches: [],
        players: ['Alminde, Olivia', 'Caswell, Emory', 'Hartley-Ward, Jaidin', 'Kobelski, Toni', 'Li, Joy', 'Macallister, Hannah', "O'Brien, Sophie", 'Roberts, Heather', 'St.Amand, Rylee', 'Tracy, Abby'],
        teamPhoto: 'assets/img/teams/2015/tanzanite.jpg',
        teamPhotoThumbNail: ''
    },
    {
        year: 2015,
        name: 'Titanium',
        division: 'Club',
        ageGroup: '16U',
        headCoach: 'Stacy Osborne',
        assistantCoaches: [],
        players: ['Almeida, Barbara', 'Bluhm, Sarah', 'Bright, Erin', 'Buczynski, Amanda', 'DiMarzio, Papillon', 'Kasprzak, Haley', 'Keener, Katelyn', 'Pavao, Olivia', 'Trant, Teagan', 'Woronko, Lina'],
        teamPhoto: 'assets/img/teams/2015/Titanium.jpg',
        teamPhotoThumbNail: ''
    },
    {
        year: 2015,
        name: 'Turquoise',
        division: 'Club',
        ageGroup: '15U',
        headCoach: 'Emily Martel',
        assistantCoaches: [],
        players: ['Bresnahan, Morgan', 'Burliss, Ryder', 'Butler, Catherine', 'Gonzalez, Yarimar', 'King, Abbie', 'LaMarr, Margaret', 'Plodzik, Jamie', 'Shin, Suhyun', 'Tang, Sunny', 'Thompson, Emma', 'Williams, Sarah'],
        teamPhoto: '',
        teamPhotoThumbNail: ''
    },
    {
        year: 2015,
        name: 'Viridian',
        division: 'Club',
        ageGroup: '16U',
        headCoach: "Dan O'Conner",
        assistantCoaches: ['Sarah Shainwald'],
        players: ['Bricknell, Jordan', 'Cheverie, Erin', 'Davies, Rachel', 'Green, Alicia', 'Jack, Julia', 'Michitson, Bryana', 'Morris, Michaela', "O'Connor, Allison", 'Riccardi, Lauren', 'Valerius, Kendra'],
        teamPhoto: '',
        teamPhotoThumbNail: ''
    }
];
